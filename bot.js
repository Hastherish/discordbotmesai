...
client.on("interactionCreate", async (interaction) => {
  if (interaction.isCommand()) {
    ...
    if (interaction.commandName === "aktif-mesai") {
      if (
        !interaction.member.roles.cache.has(
          config.commands.activeDuty.requiredRole
        )
      )
        return interaction.reply({
          content: "Bu Komutu Kullanabilmek İçin Gerekli Rolün Yok.",
          ephemeral: true,
        });
      const aktifMesaiEmbed = new EmbedBuilder()
        .setAuthor({
          name: departmentName + " AKTIF MESAIDEKILER",
          iconURL: departmentLogo,
        })
        .setColor(departmentColor);
      ...
      for (const role of roles) {
        ...
        for (const member of members) {
          for (let i = 0; i < config.settings.roles.officers.length; i++) {
            if (role.id === config.settings.roles.officers[i]) {
              if (!(await croxydb.get(`mesaiDurumu_${member.user.id}`)))
                continue;
              description += `${config.settings.emojis.on} ∙ <@${
                member.user.id
              }> | <t:${await croxydb.get(
                `mesaiGiris_${member.user.id}`
              )}:R>\n`;
            }
          }
        }
    ...
    if (interaction.commandName === "mesai-sıfırla") {
      ...
      if (type === "user") {
        ...
        if (!(await croxydb.get(`ilkMesaiDurumu_${targetID}`)))
          return interaction.reply({
        ...
        croxydb.delete(`mesaidekiler`, targetID);
        ...
        const logChannel = client.channels.cache.get(
          config.settings.channels.log
        );
        const logEmbed = new EmbedBuilder()
          .setAuthor({
            name: departmentName + " MESAI SIFIRLAMA LOG",
            iconURL: departmentLogo,
          })
          .setColor("Red")
          .setDescription(
            `\`SIFIRLAYAN YONETICI:\` <@${
              interaction.user.id
            }>\n\`SIFIRLANAN KULLANICI:\` <@${targetID}>\n\`SIFIRLANAN KULLANICININ ROZETI:\` ${getMemberBadge(
              interaction.member
            )}`
          )
          .setImage(departmentBanner);
        ...
        const logEmbed = new EmbedBuilder()
          ...
          .setColor("Red")
          .setDescription(
            `\`SIFIRLAYAN YONETICI:\` <@${interaction.user.id}>\n\`SIFIRLANAN KULLANICI:\` Bütün Kullanıcılar`
          )
          .setImage(departmentBanner);
    ...
    if (interaction.commandName === "mesai-ayarla") {
      ...
      if (!(await croxydb.has(`ilkMesaiDurumu_${target.id}`)))
        return interaction.reply({
          content: "Bu Kullanıcının Mesai Verisi Bulunmuyor.",
          ephemeral: true,
        });
      if (
        !time.includes("Gün") &&
        !time.includes("Saat") &&
        !time.includes("Dakika") &&
        !time.includes("Saniye")
      )
        return interaction.reply({
          content: "Geçersiz Mesai Süresi.",
          ephemeral: true,
        });
      ...
      if (option === "ekle") {
        await croxydb.set(
          `toplamMesai_${target.id}`,
          String(
            Number((await croxydb.get(`toplamMesai_${target.id}`)) || 0) +
              totalMs
          )
        );
        ...
        const targetDmEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `*Merhabalar, <@${
              interaction.user.id
            }> Tarafından Toplam Mesai Sürenize Ekleme Yapıldı.\nEklenen süre ${timeDifference2(
              totalMs
            )}`
          )
          .setImage(departmentBanner);
        ...
        const logEmbed = new EmbedBuilder()
          .setAuthor({
            name: departmentName + " MESAI AYARLAMA LOG",
            iconURL: departmentLogo,
          })
          .setColor("Green")
          .setDescription(
            `\`AYARLAYAN YONETICI:\` <@${
              ...
              target.id
            }>\n\`AYARLANAN KULLANICININ ROZETI:\` ${getMemberBadge(
              member
        ...
        await interaction.reply({
          content: "Kullanıcının Mesai Süresi Başarıyla Ayarlandı.",
          ephemeral: true,
        });
      } else if (option === "cikar") {
        const totalOnDutyTime = Number(
          (await croxydb.get(`toplamMesai_${target.id}`)) || 0
        );
        if (totalOnDutyTime < totalMs)
          return interaction.reply({
            content: "Kullanıcının Mesai Süresi Bu Kadar Ayarlanamaz.",
            ephemeral: true,
          });
        await croxydb.set(
          `toplamMesai_${target.id}`,
          String(totalOnDutyTime - totalMs)
        );
        ...
        const targetDmEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `*Merhabalar, <@${
              interaction.user.id
            }> Tarafından Toplam Mesai Sürenizden Çıkarma Yapıldı.\nÇıkarılan süre ${timeDifference2(
              totalMs
            )}`
          )
          .setImage(departmentBanner);
        ...
        const targetDmEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `*Merhabalar, <@${
              interaction.user.id
            }> Tarafından Toplam Mesai Süreniz Düzeltildi.\nYeni toplam mesai süreniz ${timeDifference2(
              totalMs
            )}`
          )
          .setImage(departmentBanner);
    ...
    if (interaction.commandName === "mesai-bitir") {
      ...
      if (!(await croxydb.get(`mesaiDurumu_${target.id}`)))
        return interaction.reply({
          content: "Bu Kullanıcının Mesaide Değil.",
          ephemeral: true,
        });
      ...
      if (option === "ekle") {
        const totalOnDutyTime = Number(
          new Date() -
            new Date((await croxydb.get(`mesaiGiris_${target.id}`)) * 1000) +
            Number((await croxydb.get(`toplamMesai_${target.id}`)) || 0)
        );
        ...
        const targetDmEmbed = new EmbedBuilder()
          ...
          .setDescription(
            `*Merhabalar, <@${
              interaction.user.id
            }> Tarafından Mesainiz Bitirildi. Lütfen Yeniden Mesaiye Giriş Yapınız.*\n\n${
              config.settings.emojis.clock
            } ∙ \`MESAI BITIRME ZAMANIN:\` <t:${Math.floor(
              Date.now() / 1000
            )}:R>\n\n${
              config.settings.emojis.warning
            } ∙ Unutmayınız Emek Olmadan, Yemek Olmaz.`
          )
          .setImage(departmentBanner);
        ...
        const logEmbed = new EmbedBuilder()
          .setAuthor({
            name: departmentName + " MESAI BITIRME LOG",
            iconURL: departmentLogo,
          })
          .setColor("Red")
          .setDescription(
            `\`BITIREN YONETICI:\` <@${
              ...
              target.id
            }>\n\`BITIRILEN KULLANICININ ROZETI:\` ${getMemberBadge(
              interaction.member
            )}\n\`BITIRME SEBEBI:\` ${reason}\n\`BITIRILEN KULLANICININ TOPLAM MESAISI:\` ${timeDifference2(
              totalOnDutyTime
            )}`
          )
          .setImage(departmentBanner);
  ...
  if (interaction.isButton()) {
    if (interaction.customId === "mesai_gir") {
      if (await croxydb.get(`mesaiDurumu_${interaction.user.id}`))
        return interaction.reply({
          content: "Zaten Mesaiye Giriş Yapmışsın.",
          ephemeral: true,
        });

      let toplamMesai = "Daha Önce Mesaiye Giriş Yapmamışsın.";
      if (croxydb.has(`toplamMesai_${interaction.user.id}`)) {
        toplamMesai = timeDifference2(
          Number(await croxydb.get(`toplamMesai_${interaction.user.id}`))
        );
      }
      ...
      const girEmbed = new EmbedBuilder()
        ...
        .setColor("Green")
        .setDescription(
          `*Merhabalar, Mesaiye Giriş Yaptınız Pis İllegallere Dikkat Et Dostum <3*\n\n${
            config.settings.emojis.clock
          } ∙ \`MESAI BAŞLAMA ZAMANIN:\` <t:${Math.floor(
            Date.now() / 1000
          )}:R>\n${
            config.settings.emojis.clock
          } ∙ \`TOPLAM MESAIN:\` ${toplamMesai}\n\n${
            config.settings.emojis.warning
          } ∙ Unutmayınız Emek Olmadan, Yemek Olmaz.`
        )
        .setImage(departmentBanner);
      ...
      croxydb.push(`mesaidekiler`, interaction.user.id);
      croxydb.set(
        `mesaiGiris_${interaction.user.id}`,
      ...
      if (await !croxydb.get(`ilkMesaiDurumu_${interaction.user.id}`)) {
        ...
        croxydb.set(
          `ilkMesaiTimestamp_${interaction.user.id}`,
          Math.floor(Date.now() / 1000)
        );
        croxydb.set(
          `ilkMesaiTarihi_${interaction.user.id}`,
      ...
      const logEmbed = new EmbedBuilder()
        .setAuthor({
          name: departmentName + " MESAI GIRIS LOG",
          iconURL: departmentLogo,
        })
        .setColor("Green")
        .setDescription(
          `\`GIRIS YAPAN MEMUR:\` <@${
            interaction.user.id
          }>\n\`GIRIS YAPAN MEMURUN ROZETI:\` ${getMemberBadge(
            interaction.member
          )}\n\n\`MESAI BASLAMA TARIHI:\` <t:${Math.floor(
            Date.now() / 1000
          )}:D>\n\`YAPILAN MESAI:\` ${timeDifference(
            new Date(),
            new Date(
              (await croxydb.get(`mesaiGiris_${interaction.user.id}`)) * 1000
            )
    ...
    if (interaction.customId === "mesai_cik") {
      if (!(await croxydb.get(`mesaiDurumu_${interaction.user.id}`)))
        return interaction.reply({
          content: "Zaten Mesaiden Çıkış Yapmışsın.",
          ephemeral: true,
        });
      const totalOnDutyTime = Number(
        new Date() -
          ...
          Number((await croxydb.get(`toplamMesai_${interaction.user.id}`)) || 0)
      );

      await croxydb.set(
        `toplamMesai_${interaction.user.id}`,
        String(totalOnDutyTime)
      );
      ...
      const cikEmbed = new EmbedBuilder()
        ...
        .setColor("Red")
        .setDescription(
          `*Merhabalar, Mesaiden Çıkış Yaptınız Emekleriniz İçin Teşekkür Ederiz <3*\n\n${
            config.settings.emojis.clock
          } ∙ \`YAPILAN MESAI:\` ${timeDifference(
            new Date(),
            new Date(
            ...
            config.settings.emojis.clock
          } ∙ \`TOPLAM MESAIN:\` ${toplamMesaiText}\n${
            config.settings.emojis.clock
          } ∙ \`MESAI BASLAMA TARIHIN:\` <t:${await croxydb.get(
            `mesaiGiris_${interaction.user.id}`
          )}:D>\n${
            config.settings.emojis.clock
          } ∙ \`MESAI BITIRME TARIHIN:\` <t:${Math.floor(
            Date.now() / 1000
          )}:D>\n\n${
            config.settings.emojis.warning
          } ∙ Unutmayınız Emek Olmadan, Yemek Olmaz.`
      ...
      const logEmbed = new EmbedBuilder()
        .setAuthor({
          name: departmentName + " MESAI CIKIS LOG",
          iconURL: departmentLogo,
        })
        .setColor("Red")
        .setDescription(
          `\`CIKIS YAPAN MEMUR:\` <@${
            interaction.user.id
          }>\n\`CIKIS YAPAN MEMURUN ROZETI:\` ${getMemberBadge(
            interaction.member
          )}\n\n\`MESAI BASLAMA TARIHI:\` <t:${await croxydb.get(
            `mesaiGiris_${interaction.user.id}`
          )}:D>\n\`MESAI BITIRME TARIHI:\` <t:${Math.floor(
            Date.now() / 1000
          )}:D>\n\`YAPILAN MESAI:\` ${timeDifference(
            new Date(),
            new Date(
    ...
    if (interaction.customId === "bilgilerim") {
      ...
      if (croxydb.has(`toplamMesai_${interaction.user.id}`)) {
        totalOnDutyTime = timeDifference2(
          Number(await croxydb.get(`toplamMesai_${interaction.user.id}`))
        );
      }
      let firstOnDutyTime = "Daha Önce Mesaiye Giriş Yapmamışsın.";
      if (await croxydb.get(`ilkMesaiTarihi_${interaction.user.id}`))
        firstOnDutyTime = await croxydb.get(
          `ilkMesaiTarihi_${interaction.user.id}`
        );
      let firstOnDutyTimestamp = "Daha Önce Mesaiye Giriş Yapmamışsın.";
      if (await croxydb.get(`ilkMesaiTimestamp_${interaction.user.id}`))
        firstOnDutyTimestamp = `<t:${await croxydb.get(
          `ilkMesaiTimestamp_${interaction.user.id}`
        )}:R>`;
    ...
    },
    {
      ...
      options: [
        ...
        },
        {
          name: "mesai_süresi",
          description:
            "İşlem yapılacak mesai süresi, örnek: 1 Gün, 2 Saat, 3 Dakika veya 4 Saniye",
          required: true,
          type: 3,
        },
      ],
...
function timeDifference2(ms) {
  ...
  var t =
    days + " Gün " + hours + " Saat " + minutes + " Dakika " + sec + " Saniye";
  return t;
}

async function setBotPresence(client) {
  let onDutyPoliceCount = (await croxydb.has(`mesaidekiler`))
    ? await croxydb.get(`mesaidekiler`).length
    : 0;
  let presenceType = ActivityType.Playing;
  let presenceStatus = "online";
  if (
    config.bot.status.activity === "playing" ||
    config.bot.status.activity === "PLAYING"
  )
    presenceType = ActivityType.Playing;
  if (
    config.bot.status.activity === "watching" ||
    config.bot.status.activity === "WATCHING"
  )
    presenceType = ActivityType.Watching;
  if (
    config.bot.status.activity === "listening" ||
    config.bot.status.activity === "LISTENING"
  )
    presenceType = ActivityType.Listening;
  if (
    config.bot.status.activity === "streaming" ||
    config.bot.status.activity === "STREAMING"
  )
    presenceType = ActivityType.Streaming;
  if (
    config.bot.status.activity === "competing" ||
    config.bot.status.activity === "COMPETING"
  )
    presenceType = ActivityType.Competing;
  if (
    config.bot.status.status === "online" ||
    config.bot.status.status === "ONLINE"
  )
    presenceStatus = "online";
  if (
    config.bot.status.status === "idle" ||
    config.bot.status.status === "IDLE"
  )
    presenceStatus = "idle";
  if (config.bot.status.status === "dnd" || config.bot.status.status === "DND")
    presenceStatus = "dnd";
  if (
    config.bot.status.status === "invisible" ||
    config.bot.status.status === "INVISIBLE"
  )
